...
| 端点 | 方法 | 请求 | 响应 | 备注 |
| --- | --- | --- | --- | --- |
| `/v1/events` | POST | CareEvent JSON + `Idempotency-Key` 头（必填，`[A-Za-z0-9_-]{1,128}`） | **202** `{"event_key", "status": "queued", "status_url"}` | **异步优先（2026-09-05 用户决策，已实施）**：受理与 outbox 入队同事务原子完成，工人线程执行回合；重放同键同载荷返回同一受理（202 + `Idempotent-Replay: true`） |
| `/v1/events/{key}` | GET | — | 202 `queued/processing` → 200 `{"status":"committed","response":...}`；500 `failed` | 轮询端点：读取 outbox 任务状态与最终 AgentResponse |
...
**并发同键（异步模型下的最终语义，随 2026-09-05 的异步优先决策修订）**：同键同载荷的并发提交返回**同一个 202 受理**（任务本就同一件事，无需 409 风暴）；仅两种情况报错——同键异载荷 422（请求级指纹不符），以及**底层 outbox 任务已 failed** 的键 409（提示换新键重试；受理时会交叉核对任务状态，覆盖「任务失败与键更新之间崩溃」的窗口）。崩溃恢复不再依赖对账重建：受理与入队同事务，租约过期回收 + 事件级 `event_key` 去重保证 at-least-once 投递、effectively-once 投影。
...
两个改造样例（已实施口径：`process_event` 为首个落地的执行器——异步 /events 的完整链路；
`extract_facts` / `ddi_live_fallback` 类型已在 schema 预留、执行器待 Stage 11 接入）:
(1) process_event（已实施）:  受理与入队同事务（accept_api_event）；工人线程执行
    agent.handle（event_key 由幂等键派生，重放天然去重）；完成后回填幂等键 committed。
    离线默认模式：Streamlit 直连路径不经过 outbox，行为不变。
(2) ddi_live_fallback（预留）:  _live_fallback 的 provider 调用移入工人；主路径读缓存，

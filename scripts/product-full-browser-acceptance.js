// Real browser + isolated synthetic API + real local OCR. No live patient DB.
async (page) => {
  ...
  const identity = await (await page.request.get(`${origin}/v1/acceptance/product-fixture`)).json();
  require(identity.temporary === true, 'must use isolated fixture');
  checks.isolated_fixture = true;
  ...
  await page.screenshot({path:'output/playwright/product-p5-ocr-source.png',fullPage:true});
  ...
  await page.screenshot({path:'output/playwright/product-p3-waiting-task.png',fullPage:true});
  ...
  await download.saveAs('output/playwright/product-visit-summary.html');
  ...
  await page.screenshot({path:'output/playwright/product-p3-visit-summary.png',fullPage:true});
  ...
  await page.screenshot({path:'output/playwright/product-p6-mobile.png',fullPage:true});
